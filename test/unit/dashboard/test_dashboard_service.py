...
async def test_employee_dashboard_stats():
    ...
    emp = MagicMock(id=1, name="John", role="employee", department="Eng")
    ...
    result = await service.get_dashboard_stats(employee_id=1, role="employee")
...
async def test_employee_dashboard_has_required_keys():
    ...
    result = await service.get_dashboard_stats(employee_id=1, role="employee")
    # The result should be a dict with stats
...
async def test_manager_dashboard_includes_team_data():
    ...
    result = await service.get_dashboard_stats(employee_id=1, role="manager")
    assert isinstance(result, dict)
...
async def test_admin_dashboard_includes_org_stats():
    ...
    result = await service.get_dashboard_stats(employee_id=1, role="admin")
    assert isinstance(result, dict)
