...
export default function OnboardingAuthPage() {
  ...
  const handleOAuthLogin = async (e, provider) => {
    ...
    try {
      // Mock OAuth login endpoint handles pushing to the onboarding form
      const response = await authApi.oauthLogin({ email: submitEmail, provider })
      ...
      if (err.status === 403 && err.data?.detail) {
        ...
        if (detail.onboarding_status === 'pending') {
          ...
          // Pass the email and password via session storage so they don't have to retype
          sessionStorage.setItem('onboarding_email', submitEmail)
          if (password) sessionStorage.setItem('onboarding_password', password)
    ...
    <div style={{
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <label style={{ fontSize: '0.9rem', fontWeight: 500, color: 'var(--text-muted)' }}>Create a Password</label>
            <input
              type="password"
              value={password}
          ...
          <button
            onClick={(e) => handleOAuthLogin(e, 'email')}
            disabled={loading}
            style={{
