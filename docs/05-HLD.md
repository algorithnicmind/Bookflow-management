...
| **Authentication** | Secure HttpOnly Cookie | PyJWT 2.x | Stateless secure session validation, hidden from XSS via HttpOnly flag. |
...
graph TD
    ...
    M8 --> M8A[Gemini Policy Knowledge]
    M8 --> M8B[Natural Language Query]
    ...
    style M8 fill:#4F46E5,color:#fff
```
...
### 4.8 🤖 AI Chatbot Module
**Responsibility:** Provide conversational AI assistance regarding company leave policies.

| Sub-Component | Detailed Operations |
|---------------|---------------------|
| AI Query Engine | Processes natural language questions and matches against policy context using Gemini API. |
| Policy Retrieval | Extracts relevant global policies and system limits to inform the AI model. |
...
graph LR
    ...
    subgraph Authenticated["🔐 Auth Intercepted (JWT Required)"]
        ...
        subgraph Chatbot["🤖 AI Assistant"]
            C1[POST /api/bot/chat]
        end
...
| **XSS Attacks** | Next.js auto-escapes rendered templates natively. High-grade Content-Security-Policy (CSP) headers configured at Nginx and application routing layer. JWT tokens are stored in HttpOnly cookies, completely eliminating JS access. |
| **CSRF Attacks** | Protected by a strict CSRF Middleware (`app/core/csrf.py`) using SameSite cookie directives and synchronizer token patterns. |
